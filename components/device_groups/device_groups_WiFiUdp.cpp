...
bool device_groups_WiFiUDP::beginMulticast(uint16_t port, const char* multicast_ip, const char* interface_ip) {
    ...
    if (netif == nullptr) {
        ESP_LOGE(TAG, "No WiFi STA interface found for multicast");
        return false;
    }
    
    esp_netif_ip_info_t ip_info;
    if (esp_netif_get_ip_info(netif, &ip_info) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to get IP info for multicast");
        return false;
    }
    
    if (ip_info.ip.addr == 0) {
        ESP_LOGE(TAG, "No valid IP address for multicast");
        return false;
    }
    ...
    if (setsockopt(sock_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
        ESP_LOGE(TAG, "Failed to join multicast group %s: %s", multicast_ip, strerror(errno));
        close(sock_fd);
        sock_fd = -1;
        return false;
    }
    ...
    is_connected = true;
    ESP_LOGCONFIG(TAG, "Multicast UDP socket bound to port %d, joined group %s", port, multicast_ip);
    return true;
}
...
bool device_groups_WiFiUDP::beginMulticast(const IPAddress& multicast_ip, uint16_t port) {
    ...
    struct ip_mreq mreq;
    mreq.imr_multiaddr.s_addr = htonl((multicast_ip[0] << 24) | (multicast_ip[1] << 16) | 
                                      (multicast_ip[2] << 8) | multicast_ip[3]);
    mreq.imr_interface.s_addr = INADDR_ANY;
    
    if (setsockopt(sock_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
        ESP_LOGE(TAG, "Failed to join multicast group %u.%u.%u.%u: %s", 
                 multicast_ip[0], multicast_ip[1], multicast_ip[2], multicast_ip[3], strerror(errno));
        close(sock_fd);
        sock_fd = -1;
        return false;
    }
    ...
    is_connected = true;
    ESP_LOGCONFIG(TAG, "Multicast UDP socket bound to port %d, joined group %u.%u.%u.%u", 
                  port, multicast_ip[0], multicast_ip[1], multicast_ip[2], multicast_ip[3]);
    return true;
}
